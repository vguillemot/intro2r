...
description: >
  N'oubliez pas de consulter la documentation https://ggplot2.tidyverse.org/
---
...
Utilisez la fonction `cut` de la façon suivante pour transformer la variable Fer (continue) en une variable qualitative.

```{r cutfer, exercise = TRUE}
cut(fruits$Fer, c(0, 0.2, 2))
```
...
table(cut(fruits$Fer, c(0, 0.2, 2)))
```

Remarquez comme on peut utiliser cette astuce dans un graphe !

```{r cutferplot, exercise = TRUE}
ggplot(fruits, aes(cut(Fer, c(0, 0.2, 10)))) +
  geom_bar() + 
  theme_bw()
```
...
Complétez la fonction suivante pour réaliser un boxplot de la variable `Energie1.`
...
ggplot(fruits, aes(x = Energie1, y = 0)) + 
  geom_violin() + 
  theme_bw()
```
...
ggplot(fruits, aes(VitamineC, Energie1)) +
  geom_point() + 
  theme_bw() + 
...
ggplot(fruits, aes(VitamineC, Energie1, color = Sucres)) +
  geom_point() + 
  theme_bw() + 
  labs(x = "Energie (kJ / 100g)", y = "Vitamine C Vitamine C (mg/100 g)")

ggplot(fruits, aes(VitamineC, Energie1, color = groupe)) +
  geom_point() + 
  theme_bw() + 
  labs(x = "Energie (kJ / 100g)", y = "Vitamine C Vitamine C (mg/100 g)")
```
